...
import { BookOpen, Plus, LogOut, Code2, Trophy, ShoppingBag, Zap, FileText, Folder, FolderOpen, ChevronRight, ChevronDown, FileQuestion, Calendar, Award, Users, TrendingUp, CheckCircle } from "lucide-react";
              ...
              <div className="mt-6 pt-6 border-t border-orange-200">
                ...
                <div className="space-y-3">
                  {pendingChallenges.map(challenge => {
                    const isChallenger = challenge.challenger_id === userProfile?.id;
                      ...
                      <div key={challenge.id} className={`border-2 rounded-lg p-4 ${
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex gap-2">
                            {isAccepted ? (
                              <Button
                                onClick={() => navigate(`/student/challenge/${challenge.id}/arena`)}
        ...
        {loading ? (
          ...
          <Tabs defaultValue="todo" className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 mb-6">
              <TabsTrigger value="todo" className="text-base">
                📝 To Do ({Object.keys(todoAssignments).reduce((sum, chapter) => 
                  sum + Object.keys(todoAssignments[chapter]).reduce((lessonSum, lesson) => 
                    lessonSum + todoAssignments[chapter][lesson].length, 0), 0)})
              </TabsTrigger>
              <TabsTrigger value="completed" className="text-base">
                ✅ Completed ({Object.keys(completedAssignments).reduce((sum, chapter) => 
                  sum + Object.keys(completedAssignments[chapter]).reduce((lessonSum, lesson) => 
                    lessonSum + completedAssignments[chapter][lesson].length, 0), 0)})
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="todo">
              {Object.keys(todoAssignments).length === 0 ? (
                <div className="text-center py-20">
                  <BookOpen className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">No assignments to do</h3>
                  <p className="text-gray-500">All caught up! 🎉</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {Object.keys(todoAssignments).sort().map((chapter) => {
            ...
            <TabsContent value="completed">
              {Object.keys(completedAssignments).length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-500">Complete your first assignment to see it here!</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {Object.keys(completedAssignments).sort().map((chapter) => {
                    const isChapterExpanded = expandedChapters.has(chapter + "-completed");
                    const lessons = completedAssignments[chapter];
                    
                    return (
                      <Card key={chapter} className="border-2 border-green-200 bg-green-50/30">
                        <CardHeader>
                          <div
                            ...
                            onClick={() => {
                              ...
                              if (isChapterExpanded) {
                                newExpanded.delete(chapter + "-completed");
                              } else {
                                newExpanded.add(chapter + "-completed");
                              }
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <CardTitle className="text-xl text-green-900">{chapter}</CardTitle>
                                <CardDescription className="text-green-700">
                                  {Object.keys(lessons).length} lesson{Object.keys(lessons).length !== 1 ? 's' : ''} completed
                                </CardDescription>
                              </div>
                            </div>
                            {isChapterExpanded ? (
                        ...
                        {isChapterExpanded && (
                          <CardContent className="space-y-4">
                            {Object.keys(lessons).sort().map((lesson) => {
                              const assignments = lessons[lesson];
                              const isLessonExpanded = expandedLessons.has(`${chapter}-${lesson}-completed`);
                              
                              return (
                                <div key={lesson} className="bg-white rounded-lg border border-green-200">
                                  <div
                                    className="flex items-center justify-between p-4 cursor-pointer hover:bg-green-50"
                                    onClick={() => {
                                      const newExpanded = new Set(expandedLessons);
                                      if (isLessonExpanded) {
                                        newExpanded.delete(`${chapter}-${lesson}-completed`);
                                      } else {
                                        newExpanded.add(`${chapter}-${lesson}-completed`);
                                      }
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="font-semibold text-green-900">{lesson}</span>
                                  ...
                                  {isLessonExpanded && (
                                    <div className="p-4 pt-2 grid gap-3">
                                      {assignments.map((assignment) => (
                                        <Card
                                          key={assignment.id}
                                          className="hover:shadow-md transition-shadow cursor-pointer border-green-300 bg-green-50/50"
                                          onClick={() => navigate(`/assignment/${assignment.id}`)}
                                        >
                                          <CardHeader className="pb-3">
                                            <div className="flex justify-between items-start">
                                              <CardTitle className="text-base text-green-900">{assignment.title}</CardTitle>
                                              <span className="text-xs text-green-700 bg-green-100 px-2 py-1 rounded border border-green-300 flex items-center gap-1">
                                                <CheckCircle className="w-3 h-3" />
                                                Completed
                                              </span>
                                            </div>
                                            <CardDescription className="text-sm text-green-700">{assignment.description}</CardDescription>
                                          </CardHeader>
                                          <CardContent className="pt-0">
                                            <div className="flex items-center justify-between text-sm">
