            ...
            <div className="flex gap-2">
              ...
              <Dialog modal={false} open={createDialogOpen} onOpenChange={(open) => {
                ...
                if (!open && (turtlePreviewOpen || mazeBuilderOpen)) {
                  return; // Don't close the editor if a sub-dialog is open
                }
              ...
              <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto w-[95vw]" data-testid="create-library-dialog">
                ...
                <form onSubmit={handleCreateProblem} className="space-y-4">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setNewProblem({
                              ...newProblem,
                            test_cases: [...newProblem.test_cases, { description: "", input: "", expected_output: "" }]
                            });
                          }}
                          size="sm"
                  ...
                  {newProblem.assignment_type === "turtle" && newProblem.solution_code && (
                    <div className="border-2 border-green-500/30 bg-green-500/10 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <Label className="text-lg font-semibold">🐢 Live Preview (matches student view)</Label>
                        <span className="text-xs text-slate-400">Rendered by Python turtle</span>
                      </div>
                      {newProblem.expected_turtle_image ? (
                        <div className="flex justify-center bg-white rounded-md p-2 border border-green-500/20">
                          <img
                            data-testid="lib-live-preview-img"
                            src={`data:image/png;base64,${newProblem.expected_turtle_image}`}
                            alt="Turtle output preview"
                            className="max-w-full h-auto"
                            style={{ maxHeight: 500 }}
                          />
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center bg-white/5 border border-dashed border-green-500/30 rounded-md p-10 text-center">
                          <span className="text-4xl mb-2">🐢</span>
                          <p className="text-sm text-slate-300 font-medium">No preview yet</p>
                          <p className="text-xs text-slate-500 mt-1">Click <strong>🐢 Preview Turtle Output</strong> above to run your solution code and see exactly what students will see.</p>
                        </div>
                      )}
                      <p className="text-xs text-slate-500 mt-2">
                        This preview is generated by the same Python engine that grades student submissions — what you see here is exactly what students will see as the expected output.
                      </p>
                    </div>
                  )}
                                            ...
                                            >
                                              ...
                                              <CardContent className="space-y-2">
                                                <div className="flex flex-wrap gap-2">
                                                  {problem.unit && (
                                                    <span className="px-2 py-1 bg-amber-100 text-amber-800 rounded text-xs font-bold border border-amber-300">
                                                      📦 {problem.unit}
                                                    </span>
                                                  )}
                                                  {problem.chapter && (
          ...
          <Dialog modal={false} open={editDialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto w-[95vw]">
              ...
              <form onSubmit={handleEditProblem} className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setEditingProblem({ ...editingProblem, resources_link: value });
                    }}
                    className="mt-1"
                  />
                  <p className="text-xs text-slate-500 mt-1">
                    ...
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {editingProblem.assignment_type === "turtle" && (
                        <Button
                          ...
                          size="sm"
                          variant="outline"
                          data-testid="edit-preview-turtle-btn"
                          onClick={async () => {
                            if (!editingProblem.solution_code?.trim()) {
                              toast.error("Please enter solution code first");
                              return;
                            }
                            try {
                              ...
                              if (response.data.success && response.data.image_data) {
                                setEditingProblem({ ...editingProblem, expected_turtle_image: response.data.image_data });
                ...
                {/* Turtle Preview for Edit — shows the EXACT PNG students will see (rendered by backend Python turtle). Click "🐢 Preview Turtle Output" above to refresh after code changes. */}
                {editingProblem.assignment_type === "turtle" && editingProblem.solution_code && (
                  <div className="border-2 border-green-500/30 bg-green-500/10 rounded-lg p-4">
                    ...
                    {editingProblem.expected_turtle_image ? (
                      <div className="flex justify-center bg-white rounded-md p-2 border border-green-500/20">
                        <img
                          data-testid="lib-edit-live-preview-img"
                          src={`data:image/png;base64,${editingProblem.expected_turtle_image}`}
                          alt="Turtle output preview"
                          className="max-w-full h-auto"
                          style={{ maxHeight: 500 }}
                        />
                      ...
                      <div className="flex flex-col items-center justify-center bg-white/5 border border-dashed border-green-500/30 rounded-md p-10 text-center">
                        <span className="text-4xl mb-2">🐢</span>
                        <p className="text-sm text-slate-300 font-medium">No preview yet</p>
                        <p className="text-xs text-slate-500 mt-1">Click <strong>🐢 Preview Turtle Output</strong> above to run this problem's solution code and see exactly what students will see.</p>
                      </div>
                    )}
                    <p className="text-xs text-slate-500 mt-2">
                      This preview is generated by the same Python engine that grades student submissions — what you see here is exactly what students will see as the expected output.
