  ...
  const drawCanvas = useCallback(() => {
    ...
    for (const path of pathsRef.current) {
      if (path.type === 'line') {
        ...
        console.log(`[DRAW DEBUG] Line: turtle(${path.x1}, ${path.y1}) -> (${path.x2}, ${path.y2}) | canvas(${start.x}, ${start.y}) -> (${end.x}, ${end.y})`);
        ctx.beginPath();
        ctx.moveTo(start.x, start.y);
        ctx.lineTo(end.x, end.y);
    ...
    return new Promise((resolve) => {
      ...
      switch (cmd.type) {
        ...
        case 'backward': {
          ...
          console.log(`[TURTLE DEBUG] ${cmd.type}(${cmd.value}) - heading=${turtle.heading}, from (${turtle.x.toFixed(2)}, ${turtle.y.toFixed(2)}) to (${newX.toFixed(2)}, ${newY.toFixed(2)})`);
