...
class CodeClassAPITester:
    def __init__(self, base_url="https://turtle-fix-preview.preview.emergentagent.com"):
...
%%EOF"""
    ...
    def test_microbit_endpoints(self):
        ...
        print("\n🤖 Testing Micro:bit Module Integration...")
        ...
        if lessons_response and len(lessons_response) > 0:
            ...
            required_fields = ["id", "title", "description", "starter_code", "solution_code"]
            missing_fields = [field for field in required_fields if field not in first_lesson]
            if not missing_fields:
                self.log_test("Lessons have proper structure", True)
            else:
                self.log_test("Lessons have proper structure", False, f"Missing fields: {missing_fields}")
        else:
...
"""
            ...
            # Test submission with code that matches no patterns
            print("   Testing submission with non-matching code...")
            
            non_matching_code = """# This code doesn't match Micro:bit patterns
print("Hello World")
x = 5 + 3
"""
            ...
            submission_data_2 = {
                "assignment_id": assignment_id,
                "problem_id": problem_id,
                "code": non_matching_code
            }
            
            submission_response_2 = self.run_test(
                "Submit non-Micro:bit code for pattern grading",
                "POST",
                "submissions",
                200,
                submission_data_2
            ...
            if submission_response_2:
                score_2 = submission_response_2.get("score", 0)
                print(f"   ✅ Non-matching code score: {score_2}%")
                
                # Score should be lower for non-matching code
                if score_2 < score:
                    self.log_test("Pattern grading differentiates code quality", True)
                else:
                    self.log_test("Pattern grading differentiates code quality", False, f"Non-matching score ({score_2}) not lower than matching score ({score})")
    ...
    def test_maze_challenge_api_endpoints(self):
        ...
        print("\n🧩 Testing Maze Challenge API Endpoints...")
...
"""
    ...
    def run_all_tests(self):
        ...
        # PRIORITY: Test the newly seeded Turtle curriculum problems (main focus from review request)
        self.test_turtle_curriculum()
