...
## Grade-from-blocks + save finished work (Mar 2026)
- **Grade from workspace**: `SubmissionCreate` now carries `blocks_xml`. In the blockly_turtle pattern-grading branch, when `blocks_xml` is present the grader counts block types straight from the workspace XML (`re.findall(type="...")`, mapped via `pattern_to_command` + a `turtle_` prefix fallback + an event-type map) instead of inferring from generated code. So a block counts as "used" even if it isn't wired into the run path (e.g. a heading reporter not snapped into say). Code-based counting remains the fallback when no XML is sent.
- **Save finished work**: `blocks_xml` is stored on the submission record (both blockly_turtle + ordered). Frontend `AssignmentPage.handleSubmit` now captures the current workspace XML, sends it with the submission, AND flushes a `/drafts` upsert immediately (no longer relying on the 1.5s debounce) so the finished workspace is persisted server-side. Existing draft-restore (`savedXmlPerProblem` + GET `/drafts`) already reloads a problem's blocks on revisit, so students can review how they solved earlier problems.
